...
/* PROBLEM REPORT
	...
	*	./pipex emptyfile "ls -la" "wc -l" outfile 
   				-	with emptyfile and outfile existing 
			...
			.......................................................
				-	fixed one part of this problem:
						+	the file filename was empty 
							...
							you have to add which permissions 
							a file has to get
							when creating it with open!!! 
							For more explanation view the link 
							in your google docs
							useful links file 
	...
	*	./pipex emptyfile "ls" "grep le" outfile 
				-	with emptyfile and outfile existing 
					with the right permissions
				-	it returns how one should use grep 
					and doesn't fill outfile
			...........................................................
				-	fixed this, because I was 
					splitting the wrong variable 
					to give to execve

	*	./pipex file "ls -la" "wc -l" outfile
				-	with file not existing, and 
					outfile existing with the right permissions 
				-	it executes "ls -la" as if it were "ls" 
					in the terminal, and 
	...
	*	./pipex emptyfile "ls" "grep le" newfile3
				-	emptyfile exists with right permissions, 
					and newfile3 not existing
				-	the output is different from 
					"< emptyfile ls | grep le > newfile3"
					the original also includes the newly 
					created file newfile3, 
					while mine does not include this newly created file,
					since the file isn't created until after 
					the command "ls" has run 
	*	./pipex nofile cat wc noperm
				-	nofile does not exist, and noperm 
					exists but has no permissions
				-	First it closed all the fd's and still 
					tried to execute with execve
					+	now I changed it so it doesn't execute 
						when either file is not 
						reachable or one of the duplicates failes
	*	./pipex emptyfile "cat /dev/urandom" "head -n 50" outfile
				-	emptyfile and outfile exist and 
					have the proper permissions
				-	this will break my code,
					since my parent will be waiting for the child to stop
					but that is an never ending process unless 
					the parent would close 
					the read end of the pipe (not the neatest way to stop it)
			+	I'll propably need two child processes anyway, 
				and use waitpid in the parent process
*/
